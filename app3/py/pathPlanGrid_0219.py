...
def GetPathMultiArr(contour_arr,terrainobj,vor_arr,index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[]):
    img_h=760
    img_w=1024
    scalePixToHeight=1
    ...
    for index in index_arr:
        res_arr=GridSearch(vor_arr[index],resolution,startpt,direction)
        ...
        #print("index",index)
        path3D=[]
        #print("res_arr len:",len(res_arr),"background:",len(background),"imglines",imglines)
        t_h_privous=1
        ...
        #cv2.waitKey()
    res_path=CleanPath(res_path[index_arr[0]])

    return res_path

def GetPath_test(contour_arr,terrainobj,vor_arr,index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[]):
    img_h=760
    img_w=1024
    scalePixToHeight=1
    ...
    #test polygon
    polygonExtent=UniformtoImage(vor_arr,maxInColumns,minInColumns,discol,disrow,img_h,img_w)

    #for item in polygonExtent:
        #background=cv2.polylines(background, [item],True,(255,0,0), 1)
    ...
    if len(index_arr)>0:
        index=index_arr[0]
        res_arr=GridSearch(polygonExtent[index_arr[0]],5)
        res_arr=DivideRing(res_arr,5)

        tarr=[]
        for item in res_arr:
            tarr.append([int(item[0]),int(item[1])])
        #print(tarr)

        #background=cv2.polylines(background, np.int32([tarr]),True,(255,0,0), 1)


        #imglines=UniformtoImage([res_arr],maxInColumns,minInColumns,discol,disrow,img_h,img_w)
        #print(imglines)

        #mask = cv2.polylines(background, imglines,False,(255,0,0), 1) # this will change background
        path3D=[]
        t_h_privous=1
        ...
        #cv2.imshow("background3", background)
        #cv2.waitKey()
        #print(background)
        ...
        for i in range(0,len(tarr)):
            ...
            tX=item[0] if item[0]<backgroundXLen else backgroundXLen-2
            tY=item[1] if item[1]<backgroundYLen else backgroundYLen-2
            if tX==backgroundXLen-2 and tY==backgroundYLen-2:

                pass
            else:
                #print(tX,tY)
                #t_color=background[tY,tX]
                t_h=int(background[tY,tX][1]/20)
                #print(tX,tY,background[tY,tX])
                ...
                t_scale=GetDensityFromColor(t_col)
                res_color.append([t_h,t_scale])

                t_height=baseHeight+10*(20-t_h)+5*t_scale
                #t_height=baseHeight+1*255/t_h+5*t_scale
                path3D.append([item[0],item[1],int(scalePixToHeight*t_height)])
        ...
        for i in range(0,len(tarr)):
            ...
            tX=item[0] if item[0]<backgroundXLen else backgroundXLen-2
            tY=item[1] if item[1]<backgroundYLen else backgroundYLen-2
            image = cv2.circle(background, (tX,tY), radius=6, color=(res_color[i][0]*20,0,0), thickness=-1)
    ...
    return res_path,res_color

def GetPath_backup2(contour_arr,terrainobj,vor_arr,index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[]):
    img_h=760
    img_w=1024
    scalePixToHeight=1
    ...
    if len(index_arr)>0:
        ...
        for i in range(0,len(res_arr)):
            ...
            if tX==backgroundXLen-2 and tY==backgroundYLen-2:
                ...
                #print(tX,tY)
                #t_color=background[tY,tX]
                t_h=int(background[tY,tX][1]/20)#/20+1)
                #print(tX,tY,background[tY,tX])
                ...
                #t_height=baseHeight+1*255/t_h+5*t_scale
                path3D.append([res_arr[i][0],res_arr[i][1],int(scalePixToHeight*t_height)])
                #image = cv2.circle(background, (tX,tY), radius=4, color=(t_h*10,0,0), thickness=-1)
        #cv2.imshow("background", background)
        #cv2.waitKey()
...
def GetPath(contour_arr,terrainobj,vor_arr,index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[]):
    ...
    if len(index_arr)>0:
        ...
        #res_path,res_color=CleanPathColor(path3D,res_color)
        res_path=path3D


    return res_path,res_color

def UpdateHeight(arrin,height=100,lowbar=-25,highbar=25):
...
def GetUpdatedPath(contour_arr,terrainobj,vor_arr,index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[],passpath=[]):
    ...
    arr_contours=MaskSearchedArea(imgfense,imgpasspath,img_h=760,img_w=1024)
    res_fence=ProjectImageToLocation(arr_contours,760,1024,maxInColumns,minInColumns,discol,disrow)
...
def CleanPathColor(arrin,arrcol):
    #print("arrin",len(arrin))
    res_arr=[]
    res_col=[]
    if len(arrin)>1:
        res_arr.append(arrin[0])
        res_col.append(arrcol[0])
        for i in range(1,len(arrin)):
            if arrin[i][0]!=res_arr[-1][0] or arrin[i][1]!=res_arr[-1][1] or arrin[i][2]!=res_arr[-1][2]:
                res_arr.append(arrin[i])
                res_col.append(arrcol[i])
    else:
        res_arr=arrin
        res_col=arrcol
    #print("res_arr",len(res_arr))
    return res_arr,res_col

def GetPathFromCell3D(contourarr, terrainobj, vorarr, index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857'):
    ...
    res_obj,res_color=GetPath(contour_arr,terrainobj,vor_arr,index_arr,baseHeight, resolution, 'epsg3857')
    #res_obj=CleanPath(res_obj[index_arr[0]])
    #color=[]
    return res_obj,res_color

def GetUpdatedPathFromCell3D(contourarr, terrainobj, vorarr, index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[],passpath=[]):
    # contourarr: lost person distribution
    # terrainarr: ground status of density wood or open water or openarea etc.
    # vorarr: grids
    ...
    #print(contour_arr,terrainobj,vor_arr)
    # res is an array of coordinates, which presents the 3D points of planned path polyline
    res_obj=GetUpdatedPath(contour_arr,terrainobj,vor_arr,index_arr,baseHeight, resolution, 'epsg3857',startpt,direction,passpath)
    return res_obj
