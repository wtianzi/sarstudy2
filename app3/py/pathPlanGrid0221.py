...
def GetPath_backup(contour_arr,terrainobj,vor_arr,index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[]):
    img_h=760
    img_w=1024
    scalePixToHeight=1
...
def GetPath_bak3(contour_arr,terrainobj,vor_arr,index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[]):
    img_h=760
    img_w=1024
    scalePixToHeight=1
    ...
    # res_arr [[-111.818, 34.899],[],[]]
    #print(res_arr)

    #this should uniform to extent instead of image
    print(minInColumns)
    ...
    for i in range(0,len(res_arr)-2):
        #x=int((res_arr[i][0]-minInColumns[0])*wratio)
        #y=int((res_arr[i][1]-minInColumns[1])*hratio)
        ...
        for j in range (0,ncount):
            ...
            test_arr_pts.append([tX,tY,background[tY,tX][1]])

        tX=int(0.5*tX1+0.5*tX2)
        tY=int(0.5*tY1+0.5*tY2)

        #t_h=int(background[tY,tX][1]/20)
        t_h=int(totalcol/(20*ncount))
        # terrain information
        t_pos=GetXYTerrain(tX,tY,posScaleArr,terrainobj)
        t_col=terrainImage[t_pos[1],t_pos[0]]

        t_scale=GetDensityFromColor(t_col)
        res_color.append([t_h,t_scale])

        t_height=baseHeight+100*(11-t_h)+5*t_scale
        #t_height=baseHeight+1*255/t_h+5*t_scale
        path3D.append([res_arr[i+2][0],res_arr[i+2][1],int(scalePixToHeight*t_height)])
...
def GetPath(contour_arr,terrainobj,vor_arr,index_arr,baseHeight=0, resolution = 0.0005, vorarr_spref='epsg3857',startpt=[],direction=[]):
    ...
    for i in range(0,len(res_arr)-1):
        ...
        t_scale=GetDensityFromColor(t_col)
        res_color.append([t_h-1,t_scale])
